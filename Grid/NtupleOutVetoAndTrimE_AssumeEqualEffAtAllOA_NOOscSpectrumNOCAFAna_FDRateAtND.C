           ...
           for (i_entry ; i_entry < tot_size * (i_iwritten+1); i_entry++ )
           {
             ...
             if ( ND_LAr_vtx_pos == i_ND_LAr_vtx_pos ){
               ...
               for (Double_t i_ND_LAr_dtctr_pos: a_ND_off_axis_pos_vec)
               {
                   ...
                   // Madi adding coefficients:
                   HistEtrimDetPosNoFDEventRate[i_iwritten][i_vtxX_plot-1][i_detpos-1]->Scale(1.0/validThrows * CoefficientsAtOAPos);   // * weightCAFLike[i_iwritten] * 1.0/WeightEventsAtOaPos);
                   HistEtrimDetPosWithFDEventRate[i_iwritten][i_vtxX_plot-1][i_detpos-1]->Scale(1.0/validThrows * CoefficientsAtOAPos);   // * weightCAFLike[i_iwritten] * 1.0/WeightEventsAtOaPos);
                   // for now don't write any more each individual VtxXDetPos histogram..will do so in the future probably but to speed up and empty some memory don't write it for now
